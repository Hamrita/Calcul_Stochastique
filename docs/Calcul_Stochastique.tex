...
\begin{example}
\protect\hypertarget{exm:unnamed-chunk-24}{}{\label{exm:unnamed-chunk-24} }On donne \(Y=\begin{cases}  1 \text{ avec une probabilité }\frac{1}{8}\\  2 \text{ avec une probabilité }\frac{7}{8}  \end{cases}\)
...
Donc \(\mathbb{E}\left( X|Y=y\right)= \begin{cases}  \frac{9}{4} \text{ si }Y=1 \text{ avec une prob} =\frac{3}{4}\\  \frac{18}{4} \text{ si } Y=2 \text{ avec une prob }=\frac{1}{4}  \end{cases}\).

\begin{example}
\protect\hypertarget{exm:unnamed-chunk-25}{}{\label{exm:unnamed-chunk-25} }Soit la densité jointe d'un couple aléatoire définie par: \[f(x,y)=\frac{2}{xy},\; \text{ pour }\;1<y<x<e \]
...
\begin{definition}
\protect\hypertarget{def:unnamed-chunk-26}{}{\label{def:unnamed-chunk-26} }La suite \((X_n)_{n \in \mathbb{N}}\) est une chaîne de Markov si pour tout \(n \in \mathbb{N}\) et pour tout \((x_0,x_1, \ldots,x_{n+1}) \in E^{n+2}\) tel que \(\mathbb{P}(X_n=x_n, X_{n-1}=x_{n-1},\ldots, X_0=x_0) >0\), \(\mathbb{P}(X_{n+1}=x_{n+1}|X_n=x_n,X_{n-1}=x_{n-1},\ldots, X_0=x_0)\) ne dépend que des valeurs \(n\), \(x_n\) et \(x_{n+1}\).
\end{definition}
...
\begin{proposition}
\protect\hypertarget{prp:unnamed-chunk-27}{}{\label{prp:unnamed-chunk-27} }Soit \((X_n)_{n \in \mathbb{N}}\) une chaine de Markov d'espace d'états \(E\). Pour tout \(n \in \mathbb{N}\) et pour tout \((x_0,x_1x\ldots,x_{n+1})\in E^{n+2}\), on a
\[\mathbb{P}(X_{n+1}=x_{n+1}|X_n=x_n,X_{n-1}=x_{n-1},\ldots, X_0=x_0)=\mathbb{P}(X_{n+1}=x_{n+1}|X_n=x_n)\]
\end{proposition}

\begin{definition}
\protect\hypertarget{def:unnamed-chunk-28}{}{\label{def:unnamed-chunk-28} }Une chaîne de Markov \((X_n)_{n \in \mathbb{N}}\) est dite homogène (dans le temps) si \(\mathbb{P}(X_{n+1}=x_{n+1}|X_n=x_n)\) ne dépend pas de \(n\).
\end{definition}
...
\begin{definition}
\protect\hypertarget{def:unnamed-chunk-29}{}{\label{def:unnamed-chunk-29} }Une matrice \(P=(P_{ij},i,j \in E)\) est une matrice \(\color{blue}{\mathbf{\text{stochastique}}}\), ou matrice \(\color{blue}{\mathbf{\text{de transition}}}\) si chaque ligne \(P_i=(P_{ij}, j \in E)\) est une distribution de probabilité.
\end{definition}
...
\includegraphics{Calcul_Stochastique_files/figure-latex/unnamed-chunk-32-1.pdf}

\begin{example}
\protect\hypertarget{exm:unnamed-chunk-33}{}{\label{exm:unnamed-chunk-33} }Lors d'une ronde, une méthode simple pour tromper l'ennemi et d'avance ou reculer d'une manière aléatoire, en tirant pile ou face. On désigne par \(X_n\) la position de garde à l'instant \(n\). L'évolution peut se modéliser à l'aide d'une matrice de transition
\[
 P=\begin{array}{c c} &
...
\includegraphics{Calcul_Stochastique_files/figure-latex/unnamed-chunk-34-1.pdf}
\begin{example}
\protect\hypertarget{exm:unnamed-chunk-35}{}{\label{exm:unnamed-chunk-35} }La tendance d'un marché boursier peut être haussière (H), ou baissière (B) ou en consolidation (marché en range) (C). Le graphe associé est donné comme suit:
\end{example}
...
\begin{theorem}
\protect\hypertarget{thm:unnamed-chunk-39}{}{\label{thm:unnamed-chunk-39} }Soit \(\{X_0, X_1, X_2, \ldots \}\) une CM et \(P\) sa matrice de transition carré d'ordre \(N\). Si la distribution initiale, \(\mu_0\) est donnée, alors la distribution de probabilité de \(X_n\) est donnée par \(\mu_0 P^n\).
\[
...
\begin{proposition}
\protect\hypertarget{prp:unnamed-chunk-40}{}{\label{prp:unnamed-chunk-40} }Pour toute suite \(\{x_0, x_1, \ldots , x_n\}\) dans \(E\), on a
...
\protect\hypertarget{thm:unnamed-chunk-41}{}{\label{thm:unnamed-chunk-41} \iffalse (Equations de Chapman-Kolmogorov) \fi{} }Pour tout \$(i, j) \in E\^{}2 et tout couple \((m, n)\) d'entiers positifs, on a l'identité :
\[
...
\begin{example}
\protect\hypertarget{exm:unnamed-chunk-42}{}{\label{exm:unnamed-chunk-42} }Soit une CM définie par la matrice de transition \[
  P=\left( 
  \begin{array}{cccc}
  0.2 & 0.4 & 0.1 & 0.3\\
  0 & 0.5 & 0.4 & 0.1\\
...
\begin{exercise}
\protect\hypertarget{exr:unnamed-chunk-45}{}{\label{exr:unnamed-chunk-45} } On considère une CM à trois états dont la matrice de transition est la suivante :
\[ P=\left( 
  \begin{array}{ccc}
0.6 & 0.2 & 0.2 \\
0.4 & 0 & 0.6 \\
...
\begin{exercise}
\protect\hypertarget{exr:unnamed-chunk-47}{}{\label{exr:unnamed-chunk-47} } On considère une chaîne de Markov à deux états dont la matrice de transition est la suivante :
\[ P=\left( 
  \begin{array}{cc}
0.4 & 0.6 \\
0.6 & 0.4 
...
\begin{definition}
\protect\hypertarget{def:unnamed-chunk-49}{}{\label{def:unnamed-chunk-49} }On dit que l'état \(j\) est \(\color{blue}{\mathbf{\text{accessible}}}\) à partir de l'état \(i\), s'il existe un entier \(n \geq 0\) tel que \(P^{(n)}_{ij} >0\). On note \(i \longrightarrow j\).
...
\begin{definition}
\protect\hypertarget{def:unnamed-chunk-50}{}{\label{def:unnamed-chunk-50} }Les états \(i\) et \(j\) sont dans la même classe communicante si \(i \longleftrightarrow j\).
\end{definition}

\begin{example}
\protect\hypertarget{exm:unnamed-chunk-51}{}{\label{exm:unnamed-chunk-51} }Trouvez les classes communicantes associées au diagramme de transition suivant.
\end{example}
...
\begin{definition}
\protect\hypertarget{def:unnamed-chunk-53}{}{\label{def:unnamed-chunk-53} }Une classe d'états communicante est fermée s'il est impossible de quitter
...
\begin{definition}
\protect\hypertarget{def:unnamed-chunk-54}{}{\label{def:unnamed-chunk-54} }Si la chaîne de Markov ne possède qu'une unique classe, c'est à dire que tous
...
\begin{example}
\protect\hypertarget{exm:unnamed-chunk-55}{}{\label{exm:unnamed-chunk-55} }On considère la CMD à tois états 1, 2, 3 et de matrice stochastique
\[
  P=\left( 
    \begin{array}{ccc}
    1/2 & 1/2 & 0 \\
...
\begin{example}
\protect\hypertarget{exm:unnamed-chunk-56}{}{\label{exm:unnamed-chunk-56} }Soit la CMD à quatre états définie par la matrice stochastique
\[
  P=\left( 
    \begin{array}{cccc}
    1/2 & 1/2 & 0 &0\\
...
\begin{definition}
\protect\hypertarget{def:unnamed-chunk-57}{}{\label{def:unnamed-chunk-57} }Soit \(A\) un sous ensemble de l'espace d'états \(E\). Le \textbf{temps d'atteinte} de \(A\) est la variable aléatoire \(T_A\) définie par:
\[
...
\begin{definition}
\protect\hypertarget{def:unnamed-chunk-58}{}{\label{def:unnamed-chunk-58} }Le \textbf{temps moyen d'atteinte} de \(A\), à partir de \(i\) est:
\[ m_{iA}=\mathbb{E}\left(T_A | X_0=i \right)\]
\end{definition}

\begin{example}
\protect\hypertarget{exm:unnamed-chunk-59}{}{\label{exm:unnamed-chunk-59} }On considère la chaîne suivante
\end{example}
