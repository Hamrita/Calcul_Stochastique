...
\item
  Répéter \(100000\) les instructions précédentes et stocker le résultat dans un objet que l'on appelle \texttt{mu}.
\item
...
\CommentTok{# 3}
\KeywordTok{hist}\NormalTok{(mu,}\DataTypeTok{freq =}\NormalTok{ F)}
...
\begin{center}\includegraphics{Calcul_Stochastique_files/figure-latex/tcl-1} \end{center}
...
Les citoyens de Sousse retirent de l'argent d'un distributeur de billets selon la fonction de probabilité suivante:
...
\begin{example}
\protect\hypertarget{exm:unnamed-chunk-68}{}{\label{exm:unnamed-chunk-68} }On considère la chaîne suivante
\end{example}
