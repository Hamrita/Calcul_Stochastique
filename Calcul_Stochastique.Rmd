...
```{definition}
La suite $(X_n)_{n \in \mathbb{N}}$ est une chaîne de Markov si pour tout $n \in \mathbb{N}$ et pour tout $(x_0,x_1, \ldots,x_{n+1}) \in E^{n+2}$ tel que $P(X_n=x_n, X_{n-1}=x_{n-1},\ldots, X_0=x_0) >0$, $P(X_{n+1}=x_{n+1}|X_n=x_n,X_{n-1}=x_{n-1},\ldots, X_0=x_0)$ ne dépend que des valeurs $n$, $x_n$ et $x_{n+1}$.
```
...
```
  ...
  2) Déterminer le temps moyen d'atteinte de $A=\{1,4\}$, à partie de 2.
