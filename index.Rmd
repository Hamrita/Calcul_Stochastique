...
```{r,echo=F, fig.align='center', fig.width=5, fig.height=4, warning=FALSE, message=FALSE}
library(ggfortify)
mx=cumsum(x[-(1:10)])/(11:10000)
autoplot(as.ts(mx), size=0.6)+geom_hline(yintercept = 2, colour="red",size=0.8 )+xlab("Nombre des simulations")+labs(y=paste(expression(E), "(",expression(X),"|", expression(Y=y),")"))
```
...
autoplot(as.ts(vCond[-(1:10)]), size=0.6)+geom_hline(yintercept = 0.048611, colour="red",size=0.8 )+xlab("Nombre des simulations")+labs(y=paste(expression(V), "(",expression(X),"|", expression(Y=y),")"))
```
