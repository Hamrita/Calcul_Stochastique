...
vvS=NULL
for(i in 100:10000) vvS[i]=var(SN[1:i])
...
df=data.frame(mu=mu)
gh=ggplot(df, aes(x=mu))+geom_histogram(aes(y =..density..),breaks=seq(-5,5,len=20),colour = "black", 
                   fill = "grey")+
   stat_function(fun = dnorm, args = list(mean = mean(df$mu), sd = sd(df$mu)),size=1.2,color=2)+xlab("") +ylab("")
gh
